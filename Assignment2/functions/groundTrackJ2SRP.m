function [alpha, delta, lon, lat, Y] = groundTrackJ2SRP(state, lonG0, time, body, omE, mu)

% Considering J2 and SRP effects, computes right ascension, declination, latitude,
% and longitude, given Keplerian parameters and elapsed time.
% 
% PROTOTYPE:
%   [alpha, delta, lon, lat, Y] = groundTrackJ2(state, lonG0, time, omE, mu)
% 
% INPUT:
...
%   body[2] = vector containing area to mass ratio [m^2/kg] and reflectivity [-]
%       coefficient for SRP
...
[T,Y] = ode113(@(t,y) orbitSRPJ2(t,y,mu,Re,J2,omE,body), time, [rr;vv], opts);
